---
description: Mark a batch as shipped in docs/phase-batches.md (architecture phases and M-batches), docs/review-batches.md (R-batches), or docs/polish-batches.md (U-batches), and push to main.
---
...
The user invokes this as `/strike-batch N` (architecture phases), `/strike-batch MN` (multi-league batches), `/strike-batch RN` (pre-launch review batches), or `/strike-batch UN` (premium polish batches), e.g. `/strike-batch 2`, `/strike-batch M1`, `/strike-batch R1`, `/strike-batch U3`.
...
- Otherwise reject with `"Invalid batch id '$ARGUMENTS' — expected a positive integer, M<integer>, R<integer>, or U<integer>"`.
...
5. Report back with the new commit hash and the next un-struck batch (run `grep -nE "^\| (M?R?U?[0-9]+) " $FILE | head -1` — the regex covers `| 2 `, `| M1 `, `| R3 `, and `| U4 `).

## Rules

- **Phase mode only** (numeric IDs): never strike a batch whose phases are not all marked ✅ in `wc2026-architecture.md`. Before step 3, verify by greping each phase ID in the row — if any phase is missing its `✅ YYYY-MM-DD` suffix, stop and report which one. **Skip this rule entirely in multi-league, review, and polish mode** — those items do not live in `wc2026-architecture.md`; the "shipped" signal is that the feature branch was merged to `main`. (Multi-league acceptance criteria live in `docs/multi-league-architecture.md`; that doc does not have ✅ markers in §8.)
